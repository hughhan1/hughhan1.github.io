...
import photo from '../../static/img/the-great-ocean-road/01.jpg';
...
export const TheGreatOceanRoad: React.SFC<{}> = (): JSX.Element => {
  ...
  const renderPhotos = (): JSX.Element => {
    ...
    //   const path = `${PHOTOS_PATH}/${i.toString().padStart(2, '0')}.jpg`;
    //   photos.push(<img src={require(path)} alt={`${i}`} />);
    // }
    photos.push(<img src={photo} />);
    return <div>{photos}</div>
  }

  return (
    <BlogPost title="The Great Ocean Road" date={new Date(2017, 6, 7)}>
